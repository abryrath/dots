...
(function($) {
	...
	**********************************************************************************/
	...
	//CRV IMPORTANT THE FOLLOWING FUNCTION IN THE ORIGINAL NOBLE COUNT FUNCTION.  ONCE WE CHANGED THE TEXTAREAS TO CONTENTEDITABLE DIVS, THE .val() CALL RETURNED NULL.  IN ORDER TO GET AN ACCURATE COUNT, WE HAD TO CALL THE TEXT ATTRIBUTE
	/*
function find_remaining(t_obj, max_char){
		return max_char - ($(t_obj).val()).length;
	}
*/
	//CRV THIS IS THE CUSTOM FUNCTION CRV CREATED TO GET THE TEXT LENGTH OF CONTENTEDITABLE DIVS
	function find_remaining(t_obj, max_char){
		var textCountForEmoticons = 0;
		$(t_obj).children('.emoticon').each(function(){
			textCountForEmoticons += $(this).attr('data-textvalue').length;
//			console.log('textcountforemoticons' + textCountForEmoticons);
		});
		
		var textCountForEmoji = 0;
		$(t_obj).children('.emoji').each(function(){
			textCountForEmoji += 4;
//			console.log('textcountforemoticons: ' + textCountForEmoticons);
		});
		
		
		return max_char - textCountForEmoticons - textCountForEmoji - ($(t_obj).text()).length;
	}
