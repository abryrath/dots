...
function setTimeSelectorInstructions(html)
	{
		...
		//currentDate = new Date("November 21, 2013 14:54:00"); //CRV hardcode date for testing purposes.
		//currentDate = new Date("November 21, 2014 0:54:00"); //CRV hardcode date for testing purposes.
		//currentDate = new Date("November 21, 2013 02:19:00"); //CRV hardcode date for testing purposes.
		//currentDate = new Date("November 21, 2013 19:59:00"); //CRV hardcode date for testing purposes.
		//currentDate = new Date("November 21, 2013 11:00:00"); //CRV hardcode date for testing purposes.
		//currentDate = new Date("November 21, 2013 11:54:00"); //CRV hardcode date for testing purposes.
		//currentDate = new Date("November 21, 2013 11:49:00"); //CRV hardcode date for testing purposes.
		//currentDate = new Date("November 21, 2013 11:57:00"); //CRV hardcode date for testing purposes.
		//currentDate = new Date("November 21, 2014 15:06:00"); //CRV hardcode date for testing purposes.
		//currentDate = new Date("November 21, 2013 23:54:00"); //CRV hardcode date for testing purposes.
		//currentDate = new Date("November 21, 2013 00:58:00"); //CRV hardcode date for testing purposes.
		//currentDate = new Date("November 21, 2013 00:01:00"); //CRV hardcode date for testing purposes.
		//currentDate = new Date("November 21, 2013 00:22:00"); //CRV hardcode date for testing purposes.
		//currentDate = new Date("November 21, 2013 23:48:00"); //CRV hardcode date for testing purposes. 
...
/* 				notProUserNotify('cancel-scheduled-message'); */
                alert("Sorry, you must be a MightyText Pro user to cancel this message.")
				return(false);	
			}
...
*/
						   	...
						   	console.log($('#message-id-' + reply_server.eventlist_details.ref_id));
						   	
						   	$('#message-id-' + reply_server.eventlist_details.ref_id).find('.textTimeStamp').text('Cancelling Event...');
/* 					   	} */
...
*/
				   	if($('#message-id-' + ref_id).length > 0){//CRV only buildMessageHTML if the message exists in the current DOM
					   	...
					   	console.error('newMessage html:');
				   		console.log(newMessage);
					   	//console.error(newMessage);
					   	
					   	console.log($("#message-id-"+ref_id));
					   	
					   	$('#message-id-' + ref_id).fadeOut().replaceWith(newMessage).fadeIn().each(function(){
   	                    ...
   	                    addScheduledEventCancelCapability(newContent);
			   		}
...
function createScheduledMessageLabelInDom(UNIXscheduledTime, scheduledTextRecipient)
	{
		...
		scheduledMessageLabel += '<div id="' + scheduledTextRecipient + '_label" class="scheduledLabel mightyClearfix"><span class="scheduledTime" data-unix="' + UNIXscheduledTime + '">' + cleanTimeDisplayPurposes(UNIXscheduledTime,false) + '</span><div class="closeSchedulerLabel"><strong>x</strong></div></div>';
