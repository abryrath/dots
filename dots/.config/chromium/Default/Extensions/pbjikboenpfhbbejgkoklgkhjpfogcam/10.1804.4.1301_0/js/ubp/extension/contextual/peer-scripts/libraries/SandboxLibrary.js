...
(function (SandboxLibrary) {
        ...
        Sandboxer.prototype.handle = function (requestType, args) {
            ...
            switch (requestType) {
                ...
                case this.sandboxMessagePrefix.concat("CreateSandboxById"):
                    if (args.sandboxSpecification) {
                        return this.sandboxManager.createSandboxById(args.sandboxSpecification);
                    }
                    break;
    ...
    var SandboxManager = (function () {
        ...
        SandboxManager.prototype.createSandboxById = function (sandboxSpec) {
            ...
            var sandbox = document.getElementById(sandboxId);
            if (sandbox) {
                return sandboxId;
            }
            if (targetUrl.match(/[&\?]ubpSandboxHandle=/)) {
            ...
            this.strategy.createSandbox(sandboxId, proxyUrl, cssSpec);
    ...
    var IFrameCreationStrategy = (function () {
        ...
        IFrameCreationStrategy.prototype.showSandbox = function (handle, cssSpec, pageStyleSpecification) {
            var sandbox = document.getElementById(handle);
            if (!sandbox) {
                throw new Error("Cannot show sandbox; wasn't found");
            }
            if (pageStyleSpecification && pageStyleSpecification.elementCssSelector && pageStyleSpecification.inlineCss && pageStyleSpecification.styleHandle) {
                var element = this._getElement(pageStyleSpecification.elementCssSelector);
                ...
                element.setAttribute("style", [element.dataset["ubpOldStyle"], pageStyleSpecification.inlineCss].join("; "));
            }
            sandbox.style.cssText += this._buildCssString(cssSpec);
        };
        ...
        IFrameCreationStrategy.prototype._getElement = function (elementCssSelector) {
